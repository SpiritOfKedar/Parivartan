...
import { rotateImage, type RotateDegrees } from "../lib/rotate-image";
...
export function RotateImageTool() {
  ...
  async function handleRotate() {
    ...
    try {
      const output = await rotateImage(file, degrees, format);
      setResultBlob(output.blob);
      setResultName(output.fileName);
      setPhase("done");
    } catch (cause) {
      setError(
        cause instanceof Error ? cause.message : messages.ui.couldNotRotateImage,
      );
      setPhase("error");
    }
  }
      ...
      {file && (
        <section className="space-y-6">
          ...
          <div className="space-y-2">
            <p className="text-sm font-medium text-foreground">{messages.ui.angle}</p>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            >
              {phase === "processing" ? messages.ui.rotating : messages.ui.rotateImage}
            </button>
